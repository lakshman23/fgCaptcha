...
def user_captcha(request):
    ...
    documents = Images_Fake.objects.order_by('?')[:2]
    documents1 = Images_Real.objects.order_by('?')

    # Render list page with the documents and the form
    return render_to_response(
        'registration/user_captcha.html',
...
def admin_home(request):
    return render_to_response(
    'registration/admin_homepage.html',
    )
